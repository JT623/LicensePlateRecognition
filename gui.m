...
axes(handles.axes13);imshow(p);title('中值滤波图像');
...
handles.I = I;
guidata(hObject, handles);
%J=imnoise(I,'gaussian');%加高斯噪声
J=imnoise(I,'salt & pepper',0.08);%加椒盐噪声
h=medfilt2(J);%去噪
h1=histeq(h);%增强 直方图均衡化
axes(handles.axes14);imshow(J);title('加椒盐噪声');
axes(handles.axes15);imshow(h);title('中值滤波去噪');
axes(handles.axes16);imshow(h1);title('直方图均衡化增强');
...
A1 = upcoef2('a',cA1,'bior3.7',1); %可改二维
H1 = upcoef2('h',cH1,'bior3.7',1);
V1 = upcoef2('v',cV1,'bior3.7',1);
D1 = upcoef2('d',cD1,'bior3.7',1);
Y=idwt2(A1,H1,V1,D1,'bior3.7');
axes(handles.axes18);
imshow(Y);title('一维小波重构图像');
